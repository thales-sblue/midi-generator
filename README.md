...
Para a validação manual real, após ativar o Control Surface: execute o doctor, chame `get_ableton_session`, escolha um track MIDI e scene cujo slot esteja vazio e então chame `generate_and_insert_melody` com o exemplo acima. Confirme visualmente o novo clip na Session View. Até essa sequência ser executada dentro do Live, o status deve ser reportado como **AGUARDANDO VALIDAÇÃO MANUAL EM LIVE**.
